...
export function ProjectDisplay({ project }){
    ...
    return (
        <a href={project.github_link} target="_blank" className="w-full">
            <div id="projectDisplay" className="group flex flex-col w-full bg-cover h-[25vh] bg-top-right lg:flex-row lg:h-[30vh]" style={{ backgroundImage: `url('/brand/projects/${project.id+'/'+project.bg}')` }}>
                <main className="w-full h-2/3 flex flex-col text-lg text-white justify-between p-6 space-y-2 lg:p-7 lg:w-1/2 lg:h-full lg:space-y-0">
                    ...
                    <div className="flex flex-row gap-3">
                        {project.stack.map((item, index) => (
                            <img key={index} src={`/brand/svgs/skills/${item}.svg`} alt={item} className="max-h-[3.5vh]" />
                        ))}
                    </div>
                </main>
                ...
                <span className={`w-full h-1/ px-6 flex flex-row-reverse justify-between items-center text-white hover:cursor-pointer transition-all lg:w-1/2 lg:p-7 lg:flex-row`}>
                    <img src={`/brand/projects/${project.id+'/'+project.screens}`} alt={`${project.title} screens`} className="w-[16vh] drop-shadow-xl drop-shadow-black group-hover:scale-110 transition-all duration-500 lg:w-[30vh] 2xl:w-[42vh]" />
                    
                    <img src="/brand/svgs/rchevron.svg" alt="right" width="32" className="hidden drop-shadow-sm drop-shadow-black opacity-70 group-hover:translate-x-[25%] transition-all lg:flex" />
                    <img src="/brand/svgs/skills/GitHub.svg" alt="right" width="32" className="flex drop-shadow-sm drop-shadow-black opacity-70 group-hover:translate-x-[25%] transition-all lg:hidden" />
                </span>
            </div>
        </a>
    );
