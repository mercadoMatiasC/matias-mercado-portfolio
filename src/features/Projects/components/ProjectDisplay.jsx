...
export function ProjectDisplay({ project }){
    ...
    return (
        <a href={project.github_link} target="_blank" className="w-full">
            <div className="projectDisplay group flex flex-col w-full bg-cover h-[28vh] bg-top-right lg:flex-row lg:h-[30vh] bg-white/20" style={{ backgroundImage: `url('${import.meta.env.BASE_URL}/brand/projects/${project.id+'/'+project.bg}')` }}>                <main className="w-full h-2/3 flex flex-col text-lg text-white justify-between p-6 space-y-2 lg:p-7 lg:w-1/2 lg:h-full lg:space-y-0">
                    ...
                    <div className="flex flex-row gap-3">
                        {project.stack.map((item, index) => (
                            <img loading="lazy" key={index} src={`${import.meta.env.BASE_URL}/brand/svgs/skills/${item}.svg`} alt={item} className="max-h-[3.5vh]" />
                        ))}
                    </div>
                </main>
                ...
                <span className={`w-full h-1/ px-6 flex flex-row-reverse justify-between items-center text-white hover:cursor-pointer transition-all lg:w-1/2 lg:p-7 lg:flex-row`}>
                    <img loading="lazy" src={`${import.meta.env.BASE_URL}/brand/projects/${project.id+'/'+project.screens}`} alt={`${project.title} screens`} className="w-[16vh] drop-shadow-xl drop-shadow-black group-hover:scale-110 transition-all duration-500 lg:w-[30vh] 2xl:w-[42vh]" />
